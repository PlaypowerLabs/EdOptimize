...
infoBoxWithoutIcon <- function (title, value = NULL, subtitle = NULL, 
                                color = "aqua", width = 4, href = NULL, fill = FALSE) 
{
  validateColor(color)
  colorClass <- paste0("bg-", color)
  boxContent <- div(class = "info-box", class = if (fill) 
    colorClass, div(class = "info-box-content-without-icon", span(class = "info-box-text", 
                                                                  title), if (!is.null(value)) 
                                                                    span(class = "info-box-number", value), if (!is.null(subtitle)) 
                                                                      p(subtitle)))
  if (!is.null(href)) 
...
break_col_func <- function(data, columns){
  ...
  clrs <- lapply(columns, function(x){
    if(x %in% c("leaving.Student", "bouncing.Student", "leaving.Teacher", "bouncing.Teacher",'Median Time(sec)', 'SD Score')){
...
custom_pie <- function (x, labels = names(x), edges = 200, radius = 0.8, clockwise = FALSE, 
  ...
  text_y_coord <- rep_len(c(1.2,1.6), length.out = length(labels))
  lines_coord <- list(c(1,1.2),c(1,1.08))
  ...
  for (i in 1L:nx) {
    ...
    if (!is.na(lab) && nzchar(lab)) {
      lines(lines_coord[[(i%%2)+1]] * P$x, lines_coord[[(i%%2)+1]] * P$y)
      text(1.25 * P$x, text_y_coord[i] * P$y, labels[i], xpd = TRUE, 
