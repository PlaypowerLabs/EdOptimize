...
folder_path <- file.path(getActiveProject(), 'data_prep', 'implementation_analytics_data_files')
...
walk2(test, names(test), function(x,y){
   
  saveRDS(x, file = paste0(y,'.RDS'))
})


#-----CURRICULUM INSIGHTS TAB DATA PREP-----
#----districtwise tot users data prep-----
...
file.copy(file.path(folder_path, list.files(folder_path)), file.path(getActiveProject(), 'implementation_analytics', 'data'),
          overwrite = TRUE)
