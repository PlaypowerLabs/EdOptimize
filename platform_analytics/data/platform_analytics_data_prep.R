...
#----------GENERAL DATA FOR ALL TABS IN DASHBOARD--------
...
uot_overall_by_month_rdf <- event_data_2 %>%
  mutate(month_year = paste0(month.abb[month(date)],'-',year(date))) %>%
  group_by(userid) %>%
  mutate(first = (month(date) == month(min(date))),
         last = (month(date) == month(max(date)))) %>%
  ungroup() %>%
...
#--------USAGE BY GEOGRAPHY TAB DATA PREP-----------
...
district_lat_long_rdf <- tibble(
  districtname = paste0('districtname_',1:n_distinct(event_data_2$districtname)),
  lat = maps::us.cities$lat[1:n_distinct(event_data_2$districtname)],
  long = maps::us.cities$long[1:n_distinct(event_data_2$districtname)]
)
...
#---------USAGE BY PRODUCTS TAB DATA PREP-----------
